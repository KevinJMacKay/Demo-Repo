...
$instancetype = Invoke-Command -ComputerName $Server -ScriptBlock {(New-Object System.Net.WebClient).DownloadString("http://169.254.169.254/latest/meta-data/instance-type")}
...
$header = "servername,database,vu,nopm,tpm"
	...
	if (Test-Path $LogFile)
	{
		...
		foreach ($line in $ResultsLine)
		{
			...
			Add-Content -Path $Results "$SqlServer,$instancetype,$Database,$NumUsers,$NOPM,$TPM"

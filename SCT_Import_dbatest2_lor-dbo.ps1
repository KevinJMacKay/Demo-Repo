...
param(
    ...
    [string]$SourceDatabase_lor = 'dbatest2_lor',
...
$CsvFiles = @(
    [PSCustomObject]@{
        Path      = 'C:\temp\convert\dbatest2_lor\LOR\dbatest2_lor.dbo\AURORA_POSTGRESQL_14\dbatest2_lor.dbo-AURORA_POSTGRESQL_14-Csv-report.csv'
        TableName = 'Sct_Report'
    },
    [PSCustomObject]@{
        Path      = 'C:\temp\convert\dbatest2_lor\LOR\dbatest2_lor.dbo\AURORA_POSTGRESQL_14\dbatest2_lor.dbo-AURORA_POSTGRESQL_14-Csv-report_Action_Items_Summary.csv'
        TableName = 'Sct_Report_Action_Items_Summary'
    },
    [PSCustomObject]@{
        Path      = 'C:\temp\convert\dbatest2_lor\LOR\dbatest2_lor.dbo\AURORA_POSTGRESQL_14\dbatest2_lor.dbo-AURORA_POSTGRESQL_14-Csv-report_Summary.csv'
...
$UpdateScript = "
 ...
 UPDATE [dbo].[Sct_Report_Action_Items_Summary]
   SET [BuildVersion] = '$BuildVersion',
         [Schema name] = (SELECT SUBSTRING([Schema name], CHARINDEX('.', [Schema name]) + 1, LEN([Schema name]) - CHARINDEX('.', [Schema name]))),
         [Database name] = '$SourceDatabase_lor'
 WHERE [BuildVersion] is null

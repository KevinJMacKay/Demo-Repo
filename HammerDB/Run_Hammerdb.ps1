...
$FindDB = invoke-sqlcmd -ServerInstance $Server -query $FindDBQuery 
Write-Host "Checking for database $Database on $Server."			
if ($FindDB) {
    Write-Host "Found database $Database on $Server. Proceeding." -ForegroundColor Green
}
ELSE {
    Write-Host "Database $Database NOT FOUND on $Server" -ForegroundColor Yellow   
    Write-Host "Copying $Database backup from s3 and restoring database from $RestorePath\$Database.bak."
    Invoke-Command -ComputerName $server -scriptblock { param ($Database,$RestorePath,$Server,$DataPath,$LogPath)
        ...
        if(test-path "$RestorePath\$Database.bak"){
            ...
            Restore-SqlDatabase -ServerInstance $Server -Database $Database -BackupFile "$RestorePath\$Database.bak" -RelocateFile @($RelocateData,$RelocateLog) 
        }
    } -ArgumentList $Database,$RestorePath,$Server,$DataPath,$LogPath
}
...
invoke-sqlcmd -ServerInstance $Server -inputfile $TruncQuery_File | Out-Null

# Parse Hammerdb log
Set-Location "C:\DBA_Scripts\HammerDB"
.\ParseLog_param.ps1 -Server $Server -Database $Database -LogFile $LogFile -Results $Results
